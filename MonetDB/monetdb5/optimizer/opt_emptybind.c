...
OPTemptybindImplementation(Client cntxt, MalBlkPtr mb, MalStkPtr stk, InstrPtr pci)
{
	...
	str msg = MAL_SUCCEED;
	int prevArg = 0;		// stores previous variable number in order to detect tmp bats for iterator loops
	int prevPrevArg = 0;	// dirty solution, but we don´t support more than 2 tmp bats for a single iterator loop
	...
	for (i = 0; i < limit; i++) {
		...
		// for iterator new, check if previous instruction aimed for creating a tmp variable for this iterator loop
		// if so, reset the corresponding entry in empty array with 0
		if (getModuleId(p) == iteratorRef && getFunctionId(p) == newRef) {
			if (prevPrevArg) {
				empty[prevPrevArg] = 0;
				prevPrevArg = 0;
			}
		...
		if ( getModuleId(p) == batRef && getFunctionId(p) == newRef){
			if (prevPrevArg && !prevArg) {
				prevArg = getArg(p, 0);
			} else {
				prevPrevArg = getArg(p, 0);
			}
