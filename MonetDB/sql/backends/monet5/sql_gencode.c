...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#_create_relational_function\n");
	#endif
	...
	e->card = CARD_MULTI;
	if (backend_dumpstmt(be, curBlk, r, 0, 1, NULL, NULL) < 0) {
		if (backup)
			c->curprg = backup;
		return -1;
	}
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#monet5_create_relational_function\n");
	#endif
	prop *p = NULL;
 ...
 */
static stmt *
sql_relation2stmt(backend *be, sql_rel *r, const char* nme)
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#sql_relation2stmt\n");
	#endif
	mvc *c = be->mvc;
	stmt *s = NULL;
	...
	if (!r) {
		...
		if (c->emode == m_plan) {
			...
			// use original query processing engine for internal updates
			if(nme != NULL) {
				s = output_rel_bin(be, r);
			} else {
				#ifdef _WCOJ_
					s = output_rel_bin_wcoj(be, r);
				#else
					s = output_rel_bin(be, r);
				#endif
			}
...
// TODO: nme only used for development purpose of WCOJ in order to run UPDATES on startup with old query compiler engine
int
backend_dumpstmt(backend *be, MalBlkPtr mb, sql_rel *r, int top, int add_end, const char *query, const char* nme)
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#backend_dumpstmt\n");
	#endif
	mvc *m = be->mvc;
	...
	if (query) {
		...
		#ifdef _WCOJ_DEBUG_
			TRC_DEBUG(SQL_PARSER, "escaped_q = %s\n", escaped_q);
		#endif
		...
		#ifdef _WCOJ_DEBUG_	
			TRC_DEBUG(SQL_PARSER, "q pos 1 = %s\n", instruction2str(mb, 0, q, LIST_MAL_DEBUG));
		#endif
	}
	...
	#ifdef _WCOJ_DEBUG_	
		TRC_DEBUG(SQL_PARSER, "q pos 2 = %s\n", instruction2str(mb, 0, q, LIST_MAL_DEBUG));
	#endif

	be->mvc_var = getDestVar(q);
	be->mb = mb;
	s = sql_relation2stmt(be, r, nme);

	#ifdef _WCOJ_DEBUG_	
		TRC_DEBUG(SQL_PARSER, "s = %s\n", instruction2str(mb, 0, s->q, LIST_MAL_DEBUG));
	#endif
	...
	if (top && !be->depth && (m->type == Q_SCHEMA || m->type == Q_TRANS) && !GDKembedded()) {
		...
		#ifdef _WCOJ_DEBUG_	
			TRC_DEBUG(SQL_PARSER, "q pos 3 = %s\n", instruction2str(mb, 0, q, LIST_MAL_DEBUG));
		#endif
	}
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#backend_dumpproc\n");
	#endif
	...
	if ((res = backend_dumpstmt(be, mb, r, 1, 1, be->q ? be->q->f->query : NULL, NULL)) < 0)
		goto cleanup;
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#monet5_has_module\n");
	#endif
	Client c;
	int clientID = *(int*) M;
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#monet5_resolve_function");
	#endif
	Client c;
	Module m;
	int clientID = *(int*) M;
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#backend_create_r_func\n");
	#endif
	(void)be;
	switch(f->type) {
	case  F_AGGR:
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#backend_create_py_func\n");
	#endif
	(void)be;
	switch(f->type) {
	case  F_AGGR:
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#backend_create_map_py_func\n");
	#endif
	(void)be;
	switch(f->type) {
	case  F_AGGR:
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#backend_create_c_func\n");
	#endif
	(void)be;
	switch(f->type) {
	case  F_AGGR:
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#mal_function_find_implementation_address\n");
	#endif
	mvc *o = m;
	buffer *b = NULL;
	bstream *bs = NULL;
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#backend_create_sql_func\n");
	#endif
	mvc *m = be->mvc;
	...
	#ifdef _WCOJ_DEBUG_	
		TRC_DEBUG(SQL_PARSER, "curInst 1 = %s\n", instruction2str(curBlk, 0, curInstr, LIST_MAL_DEBUG));
	#endif
	...
	if (f->res) {
		...
		if (f->type == F_UNION) {
			...
			#ifdef _WCOJ_DEBUG_	
				TRC_DEBUG(SQL_PARSER, "curInst 2 = %s\n", instruction2str(curBlk, 0, curInstr, LIST_MAL_DEBUG));
			#endif
		} else {
	...
	if (f->vararg && ops) {
		...
		for (n = ops->h; n; n = n->next, argc++) {
			...
			setVarType(curBlk, varid, type);
			#ifdef _WCOJ_DEBUG_	
				TRC_DEBUG(SQL_PARSER, "curInst 3 = %s\n", instruction2str(curBlk, 0, curInstr, LIST_MAL_DEBUG));
			#endif
		}
	} else if (f->ops) {
		int argc = 0;
		...
		for (n = f->ops->h; n; n = n->next, argc++) {
			...
			setVarType(curBlk, varid, type);
			#ifdef _WCOJ_DEBUG_	
				TRC_DEBUG(SQL_PARSER, "curInst 4 = %s\n", instruction2str(curBlk, 0, curInstr, LIST_MAL_DEBUG));
			#endif
		}
	}
	/* announce the transaction mode */
	...
	for (i = 1; i < curBlk->stop; i++) {
		...
		#ifdef _WCOJ_DEBUG_	
			TRC_DEBUG(SQL_PARSER, "p%d = %s\n", i, instruction2str(curBlk, 0, curInstr, LIST_MAL_DEBUG));
		#endif
	}
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#backend_create_func\n");
	#endif
	switch(f->lang) {
	case FUNC_LANG_INT:
	case FUNC_LANG_MAL:
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#backend_create_subfunc\n");
	#endif
	int res;
	MalBlkPtr mb = be->mb;
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#backend_create_subaggr\n");
	#endif
	int res;
	MalBlkPtr mb = be->mb;
