...
typedef struct stmt {
	...
	int tvar;		/* temporary variable assignment used for list iteration (values like str) */
	int hvar;		/* h variable used for list iteration (represents oids) */

	const char *tname;	/* table name */
...
extern stmt *stmt_append_self(backend *be, stmt *c, stmt *a);
...
extern stmt *stmt_temp_self(backend *be, stmt *old_temp);
...
extern stmt *stmt_tunion_self(backend *be, stmt *op1, stmt *op2);
extern stmt *stmt_join_crossproduct(backend *be, stmt *op1, stmt *op2, bool single);
...
extern stmt *stmt_iterate_list(backend *be, stmt *list);
extern stmt *stmt_end_iterate_list(backend *be, stmt *list);
