...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#subrel_project\n");
	#endif
	...
	for(node *n = s->op4.lval->h; n; n = n->next) {
		...
		#ifdef _WCOJ_DEBUG_
			if (s != NULL && s->q != NULL)
				TRC_DEBUG(SQL_PARSER, "s->q = %s\n", instruction2str(be->mb, 0, s->q, LIST_MAL_DEBUG));
			if (c != NULL && c->q != NULL)
				TRC_DEBUG(SQL_PARSER, "c->q = %s\n", instruction2str(be->mb, 0, c->q, LIST_MAL_DEBUG));
		#endif
	}
	s = stmt_list(be, l);
	...
	#ifdef _WCOJ_DEBUG_
		if (s != NULL && s->q != NULL)
			TRC_DEBUG(SQL_PARSER, "return s->q = %s\n", instruction2str(be->mb, 0, s->q, LIST_MAL_DEBUG));
	#endif
	return s;
}
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#rel2bin_basetable\n");
	#endif
	...
	#ifdef _WCOJ_DEBUG_	
		TRC_DEBUG(SQL_PARSER, "dels->q = %s\n", instruction2str(be->mb, 0, dels->q, LIST_MAL_DEBUG));
	#endif
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#rel2bin_table\n");
	#endif
	mvc *sql = be->mvc;
	list *l;
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#rel2bin_join\n");
	#endif
	...
	left = subrel_project(be, left, refs, rel->l);		// projection for the left sub relation of the current join
	right = subrel_project(be, right, refs, rel->r);	// projection for the right sub relation of the current join
	if (!left || !right)
		return NULL;
	 ...
	 */
	if (!list_empty(rel->exps)) {
		list *jexps = sa_list(sql->sa);	// list of joinable expressions
		sexps = sa_list(sql->sa);		// list of not joinable expressions

		split_join_exps(rel, jexps, sexps);		// split into lists of joinable and not joinable expressions
		if (list_empty(jexps)) { /* if no joinable expressions --> cross product and continue after project */
			stmt *l = bin_find_smallest_column(be, left);
		...
		if (join) {
			...
			for( en = jexps->h; en; en = en->next ) {
				...
				s = exp_bin(be, e, left, right, NULL, NULL, NULL, NULL, 0, 1, 0);	// create the join expression (left join right)
				if (!s) {
					assert(sql->session->status == -10); /* Stack overflow errors shouldn't terminate the server */
					return NULL;
				}
...
static void
subrel2hypergraph(mvc *sql, sql_rel *rel, list *vertices, list *edges) {
	node *en, *tmp;
	list *current_edge_attributes = NULL;
	Edge *current_edge = NULL;
	const char *edge_name = NULL;
	
	// traverse rel tree to obtain the join hypergraph structure
	switch (rel->op)
	{
	case op_basetable:
		current_edge_attributes = sa_list(sql->sa);

		for( en = rel->exps->h; en; en = en->next ) {
			Vertice *current_vertice = NULL;
			sql_exp *exp = en->data;
			const char *rname = exp_relname(exp) ? exp_relname(exp) : exp->l;
			const char *oname = exp->r;
			...
			Attribute *current_attribute = attribute_create(sql->sa, rname, oname);

			// search for existing vertice with current attribute name
			for(tmp = vertices->h; tmp; tmp = tmp->next) {
				Vertice *tmp_vertice = tmp->data;

				if(strcmp(oname, tmp_vertice->cname) == 0) {
					current_vertice = tmp_vertice;
					break;
				}
			}
			
			if(!current_vertice) {
				// create new attribute list and new vertice and add it to vertices list
				list *attrs = sa_list(sql->sa);
				list_append(attrs, current_attribute);
				current_vertice = vertice_create(sql->sa, oname, attrs);
				list_append(vertices, current_vertice);
			} else {
				// add attribute to existing vertice
				list_append(current_vertice->attributes, current_attribute);
			}

			list_append(current_edge_attributes, current_attribute);
		}
		current_edge = edge_create(sql->sa, edge_name, current_edge_attributes);
		current_edge->rel = rel;
		list_append(edges, current_edge);
	...
	case op_full:
		if(rel->l)
			subrel2hypergraph(sql, rel->l, vertices, edges);
		if(rel->r)
			subrel2hypergraph(sql, rel->r, vertices, edges);
		break;
	default:
		break;
	}
...
/**
 * Note that rel is required to be the closest node having type op_join to root node (type op_project)
 * 
 */
static Hypergraph * 
rel2hypergraph(mvc *sql, sql_rel *rel) {
	Hypergraph *hg = hypergraph_create(sql->sa, rel);
	list *vertices = sa_list(sql->sa), *edges = sa_list(sql->sa);

	// create the hypergraph
	subrel2hypergraph(sql, rel, vertices, edges);

	hg->original_rel = rel;
	hg->vertices = vertices;
	hg->nr_vertices = vertices->cnt;
	hg->edges = edges;
	hg->nr_edges = edges->cnt;

	return hg;
}

static stmt *
subrel_project_hg(backend *be, stmt *s, list *refs, sql_rel *rel, Edge *edge)
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#subrel_project_hg\n");
	#endif
	...
	for(node *n = s->op4.lval->h; n; n = n->next) {
		...
		// add statement to corresponding Attribute
		attr = get_attribute_from_edge(edge, c->cname);
		attr->bat_type = bat->op4.cval->type;
		attr->bat_proj_stmt = c;
		attr->current_stmt = c;
...
/** TODO: optimize loading to just load necessary columns
 *
 * The aim of this function is to load all columns that are required for the query before the actual
 * join processing, since the WCOJ algorithm uses MAL iterators. Loading them in iterators would add
 * unneccessary overhead and would lead to performance loss.
 * 
 * returns a statement list of all loaded columns
 * structure: stmt_list(stmt_list(colA1, colA2, ...), stmt_list(colB1, colB2, ...), ...)
 */
static stmt *
rel2bin_preload_columns(backend *be, Hypergraph *hg, list *refs) {
	node *n = NULL, *en = NULL;
	mvc *sql = be->mvc;
	list *l = sa_list(sql->sa);
	stmt *loaded_cols = NULL, *s = NULL;

	// iterate over all edges, load columns and create list of returned statements
	for (n = hg->edges->h; n; n = n->next) {
		Edge *edge = n->data;
		s = rel2bin_basetable(be, edge->rel);
		sql->type = Q_TABLE;
		s = subrel_project_hg(be, s, refs, edge->rel, edge);

		// append the statement to result list
		list_append(l, s);
	}

	// return obtained list of statements packed in a statement list
	return stmt_list(be, l);
}

static int
attribute_cname_cmp(Attribute *attribute, const char *cname)
{
	return strcmp(attribute->cname, cname);
}

static stmt *
rel2bin_generic_join(backend *be, Hypergraph *hg) {
	node *n = NULL, *en = NULL, *vn = NULL;
	stmt *ls = NULL, *rs = NULL, *s = NULL, *iteratorstmt = NULL;
	list *res_bats = sa_list(be->mvc->sa);
	Vertice *current_vertice = NULL, *previous_vertice = NULL, *tuple_vertice = NULL;
	Attribute *current_attribute = NULL;
	Hypergraph *hg_i = NULL, *hg_j = NULL;

	// if there is only one vertice left in the hypergraph --> intersect all relations having this attribute
	// If |V| == 1 then intersect(RF: (for all F in Edges) )
	if (hg->nr_vertices == 1) {
		Edge *edge_l = NULL, *edge_r = NULL;
		stmt *grp = NULL;

		// intersect the first two edges (relations) in any case
		// since there is just one vertice left -> read its data
		current_vertice = hg->vertices->h->data;
		n = hg->edges->h;
		edge_l = n->data;
		ls = get_statement_from_edge(edge_l, current_vertice->cname);

		if (!ls) {
			TRC_ERROR(SQL_EXECUTION, "Error intercepting relations: no attribute \"%s\" in table \"%s\" \n", current_vertice->cname, edge_l->tname);
		}
		assert(ls);

		// if there is only a single edge present for the current vertice --> return the corresponding statement without intersection
		if (hg->nr_edges == 1) {
			// check if single edge result will be used for iteration (only happens for parition I)
			if (hg->partition == 0) {
				// group the column and project the distinct values in order to ensure that the later iterator only handles each value once
				// this call compiles to group.groupdone(bat)
				grp = stmt_group(be, ls, NULL, NULL, NULL, 1);
				ls = stmt_project(be, stmt_result(be, grp, 1), ls);
			}
		...
		n = n->next;
		edge_r = n->data;
		rs = get_statement_from_edge(edge_r, current_vertice->cname);

		if (!rs) {
			TRC_ERROR(SQL_EXECUTION, "Error intercepting relations: no attribute \"%s\" in table \"%s\" \n", current_vertice->cname, edge_r->tname);
		}
		assert(rs);

		// intersect the two attribute statements and project them
		s = stmt_project(be, stmt_tinter(be, ls, rs, false), ls);

		// if there are more that 2 relations to be intersected
		if (hg->nr_edges > 2) {
			// iterate over all remaining edges (relations) to be intersected
			// TODO: examine if intersections in MAL are cascaded the correct way
			for (node *en = n->next; en; en = en->next) {
				edge_l = en->data;
				ls = get_statement_from_edge(edge_l, current_vertice->cname);
				s = stmt_project(be, stmt_tinter(be, ls, s, false), ls);
			}
		}

		// update the corresponding attribute statements
		for (node *en = n->next; en; en = en->next) {
			Edge *edge = en->data;
			update_attribute_intersection_statement(edge, current_vertice->cname, s);
		}

		// group the column and project the distinct values in order to ensure that the later iterator only handles each value once
		// this call compiles to group.groupdone(bat)
		grp = stmt_group(be, s, NULL, NULL, NULL, 1);
		return stmt_project(be, stmt_result(be, grp, 1), s);
	}

	// partition the hypergraph vertices into two sets, which then partitions the hypergraph of the current recursion into two sub hypergraphs for I and J
	hg_i = hypergraph_create(be->mvc->sa, hg->original_rel);
    hg_j = hypergraph_create(be->mvc->sa, hg->original_rel);
	partition_hypergraph(be->mvc, hg, hg_i, hg_j);

	// recursive call for partition I, the returned statement will represent the projected intersection result
	s = rel2bin_generic_join(be, hg_i);

	// reset temp bat for next iterator loop if it is not the outer-most loop (hg->partition=-1 for initial hg)
	// reset temp bat to build up tuple of this outer loop iteration
	if (hg->partition != -1) {
		current_vertice = get_vertice_from_hypergraph(hg, s->cname);
		current_vertice->tmp_result = stmt_temp_self(be, current_vertice->tmp_result);

		// Note: only works if patition builds I with |I|=1
		previous_vertice = hg_i->vertices->h->data;
		previous_vertice->tmp_candidate = stmt_temp_self(be, previous_vertice->tmp_candidate);
	}

	// handle the head of the for loop, by iterating over all tuples returned from the previous recursive call
	iteratorstmt = stmt_iterate_list(be, s);

	if (!iteratorstmt) {
		TRC_ERROR(SQL_EXECUTION, "Failed to create iterator statement!\n");
	}

	assert(iteratorstmt);

	// TODO: This implementation currently supports picking I with |I|=1. An independent implementation would require
	// to support multiple tuple statements per column
	current_attribute = get_attribute_from_hypergraph(hg_i, iteratorstmt->tname, iteratorstmt->cname);

	// project the subhypergraph
	// iterate over all edges of EJ
	for (en = hg_j->edges->h; en; en = en->next) {
		Edge *edge = en->data;

		// check if one of the current edge`s ends is in I
		for (vn = hg_i->vertices->h; vn; vn = vn->next) {
			Vertice *vertice_i = vn->data;

			// check if one of the edge`s attributes belongs to the current vertice and if it has a column of type of tuplestmt
			if ((n = list_find(edge->attributes, vertice_i->cname, (fcmp)&attribute_cname_cmp)) != NULL) {
				Attribute *attr = n->data;
				stmt *select = NULL;

				// perform a select to obtain a bat of oids that matches the current iteration´s tuple
				select = stmt_uselect(be, attr->current_stmt, iteratorstmt, cmp_equal, NULL, 0, 0);
				ls = stmt_result(be, select, 0);

				// only project attribtues from relation the belongs to the semijoin
				for (node *an = edge->attributes->h; an; an = an->next) {
					Attribute *attr_j = an->data;
					
					// only project attributes that are not of the same kind as the tuple used for semijoin
					if (strcmp(attr_j->cname, vertice_i->cname)) {
						attr_j->current_stmt = stmt_project(be, ls, attr_j->current_stmt);
					}
				}
			}
		}
	...
	// recursive call for partition J
	s = rel2bin_generic_join(be, hg_j);

	// obtain the vertice of the iterated list
	tuple_vertice = get_vertice_from_hypergraph(hg, iteratorstmt->cname);

	// else we have returned from the recursive call without intersection, thus an inner loop has been processed
	// iterate over all returned columns
	 if (s->type == st_list) {
		stmt *candidate_stmt = NULL, *result_stmt = NULL;
		n = s->op4.lval->h;
		candidate_stmt = n->data;
		result_stmt = n->next->data;

		// project the tuple column using the returned tmp_candidate stmt
		rs = stmt_const(be, candidate_stmt, iteratorstmt);

		current_vertice = get_vertice_from_hypergraph(hg_j, result_stmt->cname);
		current_vertice->result_stmt = stmt_append_self(be, current_vertice->result_stmt, result_stmt);

		if (hg->partition != -1) {
			// check needed otherwise mclient forces seg fault on tab auto complete
			if (previous_vertice) {
				previous_vertice->tmp_candidate = stmt_append_self(be, previous_vertice->tmp_candidate, candidate_stmt);
				list_append(res_bats, previous_vertice->tmp_candidate);
			}
			
			tuple_vertice->tmp_result = stmt_append_self(be, tuple_vertice->tmp_result, rs);
			tuple_vertice->tmp_result->cname = tuple_vertice->cname;
			list_append(res_bats, tuple_vertice->tmp_result);
		} else {
			tuple_vertice->result_stmt = stmt_append_self(be, tuple_vertice->result_stmt, rs);
			tuple_vertice->tmp_result->cname = tuple_vertice->cname;

			// append all result statements to final list
			for (n = hg->vertices->h; n; n = n->next) {
				current_vertice = n->data;
				list_append(res_bats, current_vertice->result_stmt);
			}
		}
	} else {
		// instead of calculating the cross product, we project the single value on a bat of oids that contains as many entries
		// as the single tuple values is required.
		const char *tname = s->tname, *cname = s->cname;

		current_vertice = get_vertice_from_hypergraph(hg_j, cname);
		current_attribute = get_attribute_from_vertice(current_vertice, tname);
		
		rs = stmt_const(be, s->op1, iteratorstmt);

		// check needed otherwise mclient forces seg fault on tab auto complete
		if (previous_vertice) {
			previous_vertice->tmp_candidate = stmt_append_self(be, previous_vertice->tmp_candidate, s->op1);
			list_append(res_bats, previous_vertice->tmp_candidate);
		}
		current_vertice->result_stmt = stmt_append_self(be, current_vertice->result_stmt, s);
		tuple_vertice->tmp_result = stmt_append_self(be, tuple_vertice->tmp_result, rs);
		tuple_vertice->tmp_result->cname = tuple_vertice->cname;
	...
	// add redo and exit for current iterator
	(void)stmt_end_iterate_list(be, iteratorstmt);

	return stmt_list(be, res_bats);
}

static stmt *
rel2bin_process_generic_join(backend *be, sql_rel *rel, list *refs)
{
	mvc *sql = be->mvc;
	list *results = sa_list(sql->sa), *cols = sa_list(sql->sa);
	node *vn = NULL, *n = NULL, *an = NULL;
	Hypergraph *hg = NULL;
	stmt *loaded_cols = NULL, *join_result = NULL;

	// convert relation to hypergraph representation
	hg = rel2hypergraph(sql, rel);
	// print_hypergraph(be->client->fdout, hg);
	
	// load columns in order to use loaded columns in iterators of MAL
	// side effect: each attribute stores its corresponding basetable statement
	loaded_cols = rel2bin_preload_columns(be, hg, refs);

	// initialize new bats to collect the results with per vertice
	for (n = hg->vertices->h; n; n = n->next) {
		Vertice *vertice = n->data;
		Attribute *attr = vertice->attributes->h->data;

		vertice->result_stmt = stmt_temp(be, &attr->bat_type);
		vertice->result_stmt->cname = vertice->cname;
		vertice->tmp_result = stmt_temp(be, &attr->bat_type);
		vertice->tmp_result->cname = vertice->cname;
		vertice->tmp_candidate = stmt_temp(be, sql_bind_localtype("oid"));
		vertice->tmp_candidate->cname = vertice->cname;
	}

	// dive into the WCOJ logic
	join_result = rel2bin_generic_join(be, hg);

	// iterate over all columns having an attribute that has been involved in the join and add the corresponding resulting BAT
	for (n = join_result->op4.lval->h; n; n = n->next) {
		stmt *s = n->data;
		
		for (vn = hg->vertices->h; vn; vn = vn->next) {
			Vertice *vertice = vn->data;

			if (strcmp(s->cname, vertice->cname) == 0) {
				for (an = vertice->attributes->h; an; an = an->next) {
					Attribute *attr = an->data;
					stmt *res = stmt_alias(be, s, attr->tname, vertice->cname);
					list_append(results, res);
				}
			}
		}
	}

	return stmt_list(be, results);
}
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#rel2bin_semijoin\n");
	#endif
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#rel2bin_union\n");
	#endif
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#rel2bin_except\n");
	#endif
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#rel2bin_inter\n");
	#endif
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#rel2bin_project\n");
	#endif
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#rel2bin_select\n");
	#endif
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#rel2bin_groupby\n");
	#endif
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#rel2bin_topn\n");
	#endif
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#rel2bin_sample\n");
	#endif
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#rel2bin_insert\n");
	#endif
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#rel2bin_update\n");
	#endif
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#rel2bin_delete\n");
	#endif
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#rel2bin_truncate\n");
	#endif
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#rel2bin_ddl\n");
	#endif
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#subrel_bin\n");
	#endif
	...
	case op_right:
	case op_full:
		s = rel2bin_process_generic_join(be, rel, refs);
		sql->type = Q_TABLE;
		break;
	case op_semi:
	case op_anti:
...
stmt *
output_rel_bin_wcoj(backend *be, sql_rel *rel )
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#output_rel_bin_wcoj\n");
	#endif
	mvc *sql = be->mvc;
	list *refs = sa_list(sql->sa);
	mapi_query_t sqltype = sql->type;
