...
dump_2(MalBlkPtr mb, const char *mod, const char *name, stmt *o1, stmt *o2)
{
	...
	if (o2->type == st_iterator_list) {
		q = pushArgument(mb, q, o2->tvar);
	} else {
...
stmt *
stmt_temp_self(backend *be, stmt *old_temp)
{
	sql_subtype t = old_temp->op4.typeval;
	int tt = t.type->localtype;
	MalBlkPtr mb = be->mb;
	InstrPtr q = newStmt(mb, batRef, newRef);
...
stmt_uselect(backend *be, stmt *op1, stmt *op2, comp_type cmptype, stmt *sub, int anti, int is_semantics)
{
	...
	if (op1->nr < 0 || op2->nr < 0 || (sub && sub->nr < 0))
		return NULL;
	if (op1->type == st_iterator_list)
		l = op1->tvar;
	else
		l = op1->nr;

	if (op2->type == st_iterator_list)
		r = op2->tvar;
	else
		r = op2->nr;
...
stmt *
stmt_tunion_self(backend *be, stmt *op1, stmt *op2)
{
	InstrPtr q = NULL;
	MalBlkPtr mb = be->mb;
...
stmt *
stmt_join_crossproduct(backend *be, stmt *op1, stmt *op2, bool single)
{
	MalBlkPtr mb = be->mb;
	InstrPtr q = NULL;
...
stmt_append(backend *be, stmt *c, stmt *a)
{
	...
	if (a->type == st_iterator_list) {
		q = pushArgument(mb, q, a->tvar);
...
stmt *
stmt_append_self(backend *be, stmt *c, stmt *a)
{
	MalBlkPtr mb = be->mb;
	InstrPtr q = NULL;
...
stmt *
stmt_iterate_list(backend *be, stmt *list) {
	MalBlkPtr mb = be->mb;
	InstrPtr q = NULL;
	int hvar, tvar;

	// unpack lists with single element
	if (list->type == st_list && list->op4.lval->cnt == 1) {
		list = list->op4.lval->h->data;
	}

	/* barrier (h,r) := iterator.new(list_to_iterate); */
	...
	(void) pushArgument(mb, q, list->nr);

	if (q) {
		stmt *s = stmt_create(be->mvc->sa, st_iterator_list);
		if(!s) {
			freeInstruction(q);
			return NULL;
		}
		s->flag = 1;		// indicates that we are facing a loop
		s->op1 = list;
		s->nr = list->nr;
		s->hvar = getArg(q, 0);
		s->tvar = tvar;
...
stmt *
stmt_end_iterate_list(backend *be, stmt *list) {
	MalBlkPtr mb = be->mb;
	InstrPtr q = NULL;
	int hvar = list->hvar, tvar = list->tvar;

	/* redo (h,r):= iterator.next(list); */
