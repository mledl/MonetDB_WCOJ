...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "#SQLparser");
	#endif
	...
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_PARSER, "c->query = %s", c->query);
	#endif
	...
	if (c->query == NULL) {
		...
		#ifdef _WCOJ_DEBUG_
			TRC_DEBUG(SQL_PARSER, "execution mode = %d", m->emode);
		#endif
		...
		if (m->emode != m_prepare) {
			...
			if (backend_dumpstmt(be, c->curprg->def, r, 1, 0, c->query, NULL) < 0)
				err = 1;
			else
				opt = 1;
		} else {
	...
	if (err == 0) {
		...
		if (!err) {
			...
			#ifdef _WCOJ_DEBUG_
				TRC_DEBUG(SQL_PARSER, "pushEndInstruction = %s", instruction2str(c->curprg->def, 0, *c->curprg->def->stmt, LIST_MAL_DEBUG));
			#endif
