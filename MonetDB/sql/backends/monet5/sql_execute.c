...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_EXECUTION, "#SQLsetTrace\n");
	#endif
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_EXECUTION, "#SQLrun\n");
	#endif
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_EXECUTION, "#SQLstatementIntern\n");
	#endif
	...
	*o = *m;
	...
	while (msg == MAL_SUCCEED && m->scanner.rs->pos < m->scanner.rs->len) {
		...
		/* generate MAL code */
			...
			if (backend_dumpstmt(be, c->curprg->def, r, 1, 1, NULL, nme) < 0)
				err = 1;
			be->depth--;
#ifdef _SQL_COMPILE
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_EXECUTION, "#SQLengineIntern\n");
	#endif
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_EXECUTION, "#RAstatement\n");
	#endif
	...
	if (rel) {
		...
		setVarType(c->curprg->def, 0, 0);
		if (backend_dumpstmt(b, c->curprg->def, rel, 1, 1, NULL, NULL) < 0) {
...
{
	#ifdef _WCOJ_DEBUG_
		TRC_DEBUG(SQL_EXECUTION, "#RAstatement2\n");
	#endif
